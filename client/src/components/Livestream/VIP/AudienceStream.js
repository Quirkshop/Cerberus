...
var pubs = [null, null, null, null, null, null];
...
const AudienceStream = (props) => {
  // Mixpanel.track("VIP Audience Stream");
  ...
  const init = async () => {
    Mixpanel.track("VIP Audience Stream Initiated");
    client.connect();
    client.onConnected(async (props) => {
      ...
      peerConnection.setRemoteDescription(
        new RTCSessionDescription(joinResult.jsep)
      );
      ...
      publishResult = await roomHandle.start({
        request: "start",
        room: roomID,
        jsep: answer,
      });
      ...
      // newRemoteFeed(hostFeed, null, null);
      ...
      endButton.addEventListener("click", (e) => {
        ...
        setLive(false);
        Mixpanel.track("VIP Stream Exit");
  ...
  const newRemoteFeed = async (id, audio, video) => {
    ...
    console.log(result);
    // //debugger;
    let subResult = await listener.start({
      request: "start",
      room: roomID,
      jsep: answer,
    });
  ...
  const onTrack = (event) => {
    ...
    if (hostStream === null && rs0 === false) {
      hostStream = event.streams[0];
      rs0Key = getRandomInt(0, 100);
      console.log(hostStream);
    } else if (hostStream !== null && rs0 === false) {
      hostStream = event.streams[0];
      rs0 = true;
      rs0Key = getRandomInt(0, 100);
      console.log(hostStream);
    ...
    <>
      <div className={classes.root}>
        ...
        <Grid container spacing={1}>
          <div className={classes.player_wrapper} key={rs0Key}>
            <Grid item xs={3} key={rs0Key}>
              {hostStream && hostStream.active && (
                <ReactPlayer
                  key={rs0Key}
                  volume={0.99}
                  muted={false}
